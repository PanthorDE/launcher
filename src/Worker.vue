...
export default defineComponent({
  ...
  methods: {
    ...
    downloadFileRecursive(
      to_download: Array<ModFile>,
      index: number,
      callback: Function,
      size_done: number = 0,
      total_size: number = 0,
      last_speed_calc: number = Date.now(),
      last_speed_size_done: number = 0
    ) {
      let request_download = new Promise((resolve, reject) => {
        ...
        http
          .get(this.mod.files + file.RelativPath, (response) => {
            ...
            response.on('data', (chunk) => {
              ...
              if (last_speed_calc + 1000 < Date.now()) {
                this.worker_status.fileop_speed = Math.round(
                  (size_done + streamed_chunkslength - last_speed_size_done) / ((Date.now() - last_speed_calc) / 1000)
                );
                this.worker_status.fileop_time_remaining = Math.round(
                  ((total_size - size_done) / this.worker_status.fileop_speed) * 1000
                );
