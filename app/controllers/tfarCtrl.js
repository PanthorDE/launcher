    ...
    switch (args.type) {
      ...
      case 'update-dl-progress-file-done':
        ...
        if (!shell.openItem(args.filePath)) {
          ...
          let stream = fs.createReadStream(args.filePath).pipe(unzip.Extract({path: app.getPath('downloads') + '\\Panthor'}))
          stream.on('close', () => {
            try {
            ...
            shell.showItemInFolder(app.getPath('downloads') + '\\Panthor')
          })
        }
        break
    }
